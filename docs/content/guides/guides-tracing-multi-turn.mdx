...
Pass `thread_id` to `DeepEvalInstrumentationSettings` when constructing your agent. Every `run_sync` or `run` call on that agent instance is tagged with the same thread and grouped accordingly.
...
agent = Agent(
    ...
    instrument=DeepEvalInstrumentationSettings(thread_id=thread_id),
)
...
<FAQs
  qas={[
    ...
    {
      question:
        "What's the difference between a trace and a thread in DeepEval?",
      answer: (
        <>
    ...
    },
    {
      ...
      answer: (
        <>
          Use <code>update_current_span(retrieval_context=...)</code> inside the
          retriever step of that turn. This makes multi-turn RAG metrics like{" "}
          <code>TurnFaithfulnessMetric</code> work without extra wiring.
        </>
      ),
    },
    {
      ...
      answer: (
        <>
          Call{" "}
          <code>evaluate_thread(thread_id=..., metric_collection=...)</code>{" "}
